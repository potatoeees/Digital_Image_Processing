# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 00:34:16 2023
...
vid = cv2.VideoCapture("singapore.mp4") 
...
output = cv2.VideoWriter("singapore_processed.avi", cv2.VideoWriter_fourcc(*"MJPG"), 30.0, (1280,720))
...
for frame_count in range(0, int(total_no_frames)): # To loop through all the frames
    ...
    # Detect face and perform blurring
    ...
    for (x, y, w, h) in faces: # To loop through all the detected faces.
        ...
        blurred_face = cv2.GaussianBlur(face_roi, (25, 25), 0) # Apply Gaussian blur to the face region
